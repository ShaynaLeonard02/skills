#!/usr/bin/env python3
"""
Extract fully Read-Only registers from MMG AL register CSV files (bz2-compressed).

A register is included only when EVERY one of its fields carries an RO access type.
Access types treated as RO: RO, RO/V, RO/C
...
Usage:
    python3 extract_ro_registers.py [--src <path>] [--out <path>] [--als <path>]
...
RO_ACCESS_TYPES = {"RO", "RO/V", "RO/C"}


def parse_args():
    parser = argparse.ArgumentParser(description="Extract fully-RO registers from AL CSV files.")
    parser.add_argument(
        "--src",
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(os.getcwd(), "read_only_csr"),
        help="Output directory for per-IP CSV files (default: <cwd>/read_only_csr)",
    )
    parser.add_argument(
        "--als",
        nargs="+",
...
def main():
    ...
    for filepath in files:
        ...
        try:
            ...
            # Keep only registers where EVERY field is RO, optionally filtered
            # to those present in the AL set.
            fully_ro_rows = [
                row
                for (rf, reg), fields in reg_fields.items()
                if (not al_set or (rf, reg) in al_set)
                and all(f.get("Access", "").strip() in RO_ACCESS_TYPES for f in fields)
                for row in fields
            ]

            if not fully_ro_rows:
                skipped += 1
            ...
            with open(out_path, "w", newline="", encoding="utf-8") as out_f:
                ...
                writer.writeheader()
                writer.writerows(fully_ro_rows)

            processed += 1
            print(f"  [{processed:>3}] {ip_name}: {len(fully_ro_rows)} fields -> {os.path.basename(out_path)}")

        except Exception as e:
            print(f"  ERROR {ip_name}: {e}")

    print(f"\nDone. {processed} IPs written, {skipped} IPs had no fully-RO registers.")
